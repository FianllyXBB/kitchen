...
CREATE TABLE `users` (
  ...
  `phonenumber` char(11) DEFAULT NULL COMMENT '手机号码11位可以为空',
...
#基本套餐表
CREATE TABLE `combo` (
  `comboid` char(32) NOT NULL COMMENT '基本套餐主键',
  `comboname` varchar(50) NOT NULL COMMENT '套餐名字',
  `comboimage` varchar(200) NOT NULL COMMENT '套餐图片',
  `combodescription` varchar(300) NOT NULL COMMENT '套餐详细介绍(300位以内)',
  `comboprice` decimal(5,1) NOT NULL COMMENT '价格',
  `combonum` int(4) NOT NULL COMMENT '总份数',
  `comborenum` int(4) NOT NULL COMMENT '剩余份数',
  `show` tinyint(1) DEFAULT NULL COMMENT '保留字段，0删除，1未删除',
  PRIMARY KEY (`comboid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `orderitems` (
  ...
  `comboid` char(32) DEFAULT NULL COMMENT '套餐id，实际为外键关系，但可以为空',
